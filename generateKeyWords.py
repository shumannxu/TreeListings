...
def find_key_words(title: str, desc: str, categories: List[str], num_key_words: int) -> List[str]:
    # preprocessing
    ...
    prompt = "'" + ','.join(keywords) + "'." + "The preceding texts were the description of a product or a service with the title," + title + ", posted on a listing on an e-commerce website. Generate " + str(num_key_words) + " key words, where each is max 2 words long, that are most relevant keywords that describe this product or service OR most relevant keywords that describe the category or genre that this product or service strongly relates to. The generated keywords should not be a direct match with any words in the title. Please respond in the format provided after this sentence. KEY; [xxxx, xxxx, xxxxx, xxxx, xxxxx, xxx, xxx, xxx, xxx, xxx]"
    ...
    keywords_list = [word.strip().lower() for word in keywords_str.split(",")][:num_key_words]
...
def EXAMPLE_USAGE():
    ...
    EXAMPLES = [
        ("Apple iPhone 11 Max 256GB - Excellent Condition!", "Up for sale is my trusted Apple iPhone 11 Max with 256GB storage capacity. This smartphone is in excellent condition, meticulously cared for, and fully functional. It comes with a stunning 6.5-inch Super Retina XDR display, delivering vibrant colors and crisp images. The battery health is still impressive, ensuring long hours of uninterrupted usage. Whether you're capturing precious memories with its advanced camera system or enjoying seamless performance with its powerful A13 Bionic chip, this iPhone 11 Max is sure to exceed your expectations. Don't miss out on this opportunity to own a premium device at a great price!", ["Electronics"]),
        ("Stanford Hat (Black)", "Used, tennis cap. Perfect for a sunny day. Also perfect for sports.", ["Apparel","Sporting Goods"]),
        ("Powerful Gaming Laptop: ASUS ROG Strix Scar 15", "Elevate your gaming experience with the ASUS ROG Strix Scar 15 gaming laptop. Packed with high-performance components, this laptop is built to deliver uncompromising gaming performance. Featuring an Intel Core i9 processor and NVIDIA GeForce RTX 3080 graphics, you'll enjoy smooth gameplay with stunning visuals. The 15.6-inch Full HD display boasts a fast refresh rate and ultra-low response time, ensuring you never miss a moment of the action. With ample storage and lightning-fast SSD, you can store your entire game library and enjoy lightning-fast load times. Plus, the customizable RGB keyboard adds a touch of style to your gaming setup. Whether you're conquering the latest AAA titles or streaming your favorite games, the ASUS ROG Strix Scar 15 is your ultimate gaming companion. Get ready to dominate the battlefield and achieve victory in style!", ["Electronics"]),
        ("Small Heater", "Can adjust the strength of heat and can also act as a mini fan. Used for about 1 year, perfect condition", ["Electronics", "Office Supplies","Home Goods"]), 
        ("Black Pearl Necklace", "Not used and real black pearl from Gifu, Japan", "Apparel,"),
        ("stuffed animals", "stuffed animals used", "Toys & Games,"),
        ("Bed mattress", "new condition, soft and comfy", ["Home Goods"]),
        ("Math Tutoring", "Tutoring for anyone for up to 5 hours per week on math", ["Services"]),
        ("Leasing Room in EVGR", "Leasing one-room single on the 4th floor of EVGR. The room comes with a bathroom and a kitchen.", ["Property Rentals"])
    ]
    # running main function 
    for i in range(len(EXAMPLES)):
        (title, desc, categories) = EXAMPLES[i]
        num_keywords_to_generate = 5
        keywords_list = find_key_words(title, desc, categories, num_keywords_to_generate)
        
""" Uncomment the line below to test the main function """
# EXAMPLE_USAGE()
